...
void indicateConversionValue(void)
{
    ...
    unsigned short smootheValue = getsmootheValue(&g_adcMovingAverage);

    if(smootheValue < LIMIT_LOW)       /* LED1 lights up if the conversion value is smaller than 0x555 */
    {
        IfxPort_setPinLow(LED1);
        IfxPort_setPinHigh(LED2);
    }
    else if(smootheValue > LIMIT_HIGH) /* LED2 lights up if the conversion value is greater than 0xAAA */
    {
        IfxPort_setPinHigh(LED1);
...
uint16 getsmootheValue(MovingAverage_t *ma)
{
